...
Bienvenidos a este RMarkdown en donde analizaremos las ventas de una pizzería durante el periodo del año 2015, con el fin de encontrar aquellos productos más consumidos por los clientes y plantear reglas de asociación **las pizzas más elegidas entre los clientes.**
...
Como vemos, a nivel mensual no hay mucha diferenia en el nivel de ventas, sin embargo, si lo vemos semana a semana, podemos notar que los días vienres son aquellos en donde más flujo de clientes hay, por lo que este dato es interesante tenerlo en cuenta en el timing para lanzar las promociones.
...
```{r sup&conf,echo=F, message=FALSE, warning=FALSE}
...
order <- c(rules_sup1_conf10[9],rules_sup1_conf10[22],rules_sup1_conf10[21],rules_sup1_conf10[38],rules_sup1_conf10[39],rules_sup1_conf10[23],rules_sup1_conf10[1],rules_sup1_conf10[62],rules_sup1_conf10[24],rules_sup1_conf10[42])
...
```{r plot_inspect,echo=FALSE}

plot(order, measure = c("support", "lift"), shading = "confidence",jitter = 0)
```
