...
# Análisis explorato4 por prueba

Bienvenidos a este Rmarkdown en donde analizaremos el dataset que comprende a los 200 nadadores más rapidos por cada prueba, este ranking se desarrola a partir de los mejores tiempos realizados en cada evento profesional realizado en albercas de 50 metros. Con este conjunto de datos vamos a analizar:
...
-   El evento con lás mejores marcas.
...
data <- data %>% 
  ...
  arrange(desc(Event.description))
...
data %>% 
  ...
  transmute(
    ...
    id.Event.Name = as.integer(Event.Name),
  ) %>% 
  summarise(
    Count = sum(id.Event.Name/id.Event.Name)
  ) %>% 
